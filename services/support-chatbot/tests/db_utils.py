...
The DB-backed property (Task 8.10 / Property 36) needs a real PostgreSQL. There is
no guaranteed local Postgres in every environment, so these helpers resolve a test
database URL from the environment and probe it with a **short timeout**, so a test
SKIPS cleanly (never hangs or hard-fails) when no database is reachable. The
orchestrator runs the test against an ephemeral ``postgres:16`` afterward.

...
The Capstone unit/property tests exercise persistence through a **stubbed**
``HistoryRepository`` (no real Database needed), so this module is provided for
parity with the other services and for the DB-backed history example test: it
resolves a test database URL from the environment and probes it with a **short
timeout** so a test SKIPS cleanly (never hangs or hard-fails) when no database is
reachable. The orchestrator runs DB-backed tests against an ephemeral
``postgres:16`` provided via ``TEST_DATABASE_URL``.
...
The URL is captured at **import time** (during collection) so it survives any env
isolation a fixture might apply.
"""
